...
namespace Unit04.Game.Casting
{
    ...
    public class Cast
    {
        ...
        public void AddActor(string group, Actor actor)
        {
            ...
            {// Si no hay una lista con ese nombre,crea una nueva lista bajo el key group
                _actors[group] = new List<Actor>();
            }

            if (!_actors[group].Contains(actor))
            {// Si no hay elemento con ese nombre crea uno nuevo bajo el key group.
                _actors[group].Add(actor);
            }
        }

        /// <summary>
        /// Gets teh actors in the given group. Returns an empty list if there aren't any.
        /// </summary>
        /// <param name="group">The group name.</param>
