    ...
    public class RaylibVideoService : VideoService
    {
        ...
        public void DrawImage(Casting.Image image, Casting.Point position)
        {
            ...
            if (!_textures.ContainsKey(filename))
            {
                ...
                _textures[filename] = loaded;
            }
            Raylib_cs.Texture2D texture = _textures[filename];
            int x = position.GetX();
            int y = position.GetY();
        ...
        public void DrawText(Casting.Text text, Casting.Point position)
        {
            ...
            if (!_fonts.ContainsKey(filename))
            {
                ...
                _fonts[filename] = loaded;
            }
            Raylib_cs.Font font = _fonts[filename];

            x = RecalculateTextPosition(font, value, size, x, alignment);
            Raylib_cs.Color raylibColor = ToRaylibColor(color);
        ...
        {
            List<string> filters = new List<string>() { "*.jpg", "*.gif", "*.png", "*jpeg", "*bmp" };
            List<string> filepaths = GetFilepaths(directory, filters);
            foreach (string filepath in filepaths)
            {
        ...
        private int RecalculateTextPosition(Font font, string text, int size, int x, int alignment)
        {
